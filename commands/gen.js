...
module.exports.run = (bot, message, args, gen) => {
    if(!gen.hasAccess(message.member)) return message.channel.send({embed:{title:`You need to invite 6 members to use this gen!`, color:bot.color}})
    ...
    .setTitle('RobloxGen')
    .addField('Type', type)
    .addField('Username',alt.split(':')[0],true)
